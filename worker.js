...
const HTML = [
  ...
  '  <style>',
  '    :root {',
  '      --bg:#f5f7fa; --card:#fff; --text:#1a1a2e; --text-secondary:#6b7280;',
  '      --accent:#4f6ef7; --accent-light:#eef1ff; --danger:#ef4444; --success:#10b981;',
  '      --border:#e5e7eb; --shadow:0 2px 8px rgba(0,0,0,0.06); --radius:12px; --radius-sm:8px;',
  '    }',
  ...
  '    .file-icon.folder { background:#fef3c7; } .file-icon.image { background:#dcfce7; } .file-icon.video { background:#fee2e2; } .file-icon.text { background:#e0e7ff; }',
  ...
  '    .btn-text { padding:4px 10px; font-size:12px; border:1px solid var(--border); background:#f9fafb; border-radius:6px; cursor:pointer; color:var(--text-secondary); -webkit-tap-highlight-color:transparent; }',
  '    .btn-text:hover { background:var(--accent-light); color:var(--accent); } .btn-text.danger:hover { background:#fee2e2; color:var(--danger); }',
  ...
  '    .btn-primary { background:var(--accent); color:#fff; } .btn-secondary { background:#f3f4f6; color:var(--text); } .btn-danger { background:var(--danger); color:#fff; }',
  '    .empty-state { text-align:center; padding:60px 20px; color:var(--text-secondary); } .empty-state .icon { font-size:48px; margin-bottom:12px; }',
  ...
  '    .task-panel.open { transform:translateX(0); } .task-panel-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; }',
  '    .task-item { padding:10px; border-radius:var(--radius-sm); background:#f9fafb; margin-bottom:8px; border:1px solid var(--border); }',
  '    .task-name { font-size:13px; font-weight:600; word-break:break-all; } .task-status { font-size:11px; color:var(--text-secondary); margin-top:4px; }',
  '    .progress-bar { height:4px; background:#e5e7eb; border-radius:2px; margin-top:6px; overflow:hidden; } .progress-fill { height:100%; background:var(--accent); } .progress-fill.completed { background:var(--success); } .progress-fill.failed { background:var(--danger); }',
  '    .task-backdrop { position:fixed; inset:0; background:rgba(0,0,0,0.3); z-index:240; opacity:0; pointer-events:none; transition:opacity 0.3s; } .task-backdrop.open { opacity:1; pointer-events:auto; }',
  ...
  '    .detail-header { display:flex; align-items:center; gap:12px; margin-bottom:16px; } .detail-title { font-size:18px; font-weight:700; word-break:break-all; }',
  '    .detail-meta { font-size:13px; color:var(--text-secondary); margin-bottom:16px; } .detail-actions { display:flex; flex-wrap:wrap; gap:8px; margin-bottom:16px; }',
  '    .preview-container { margin-top:16px; } .preview-container img { max-width:100%; border-radius:8px; } .preview-container video, .preview-container audio { width:100%; border-radius:8px; }',
  ...
  '    .editor-wrapper { border:1px solid #ccc; z-index:100; } .toolbar-container { border-bottom:1px solid #ccc; } .editor-container { height:400px; }',
  ...
  '        } else {',
  '          openDetail(findItemInTree(fileTree, id));',
  '        }',
  '      }',
  '    });',
  '',
  ...
  '      var items = getCurrentFolderItems();',
  '      for (var i = 0; i < items.length; i++) { if (items[i].name === name) { showToast("同名文件已存在"); return; } }',
  ...
  '      item.name = newName; item.updatedAt = Date.now();',
  '      saveTree().then(function() { closeModal(); renderFileList(); showToast("重命名成功"); if (currentView === "detail" && currentDetailItem && currentDetailItem.id === itemId) renderDetail(); });',
  '    }',
  '',
  ...
  '        if (item.type === "file" && item.chunks > 0) { api("/api/delete-content", { method:"POST", body: { fileId: item.id, chunks: item.chunks } }).catch(function(){}); }',
  '        removeItemFromTree(fileTree, itemId);',
  '        saveTree().then(function() { renderFileList(); showToast("已删除"); if (currentView === "detail" && currentDetailItem && currentDetailItem.id === itemId) closeDetail(); });',
  '      }',
  '    }',
  '',
  '    // ========== 上传 ==========',
  ...
  '        (function(file) { queue = queue.then(function() { return uploadSingleFile(file, file.name, uploadPath); }); })(files[i]);',
  '      }',
  '      queue.then(function() { showToast("上传完成"); loadTree(); }).catch(function(e) { showToast("上传失败: " + e.message); });',
  '    }',
  '',
  ...
  '      closeModal();',
  '      api("/api/offline", { method:"POST", body: { url: url, savePath: currentPath } }).then(function() { showToast("离线下载任务已创建"); loadTasks(); }).catch(function(e) { showToast("创建任务失败: " + e.message); });',
  '    }',
  '',
  '    // ========== 任务管理 ==========',
  '    function loadTasks() {',
  '      return api("/api/tasks").then(function(data) { tasks = data.tasks || []; renderTasks(); });',
  '    }',
  '',
  ...
  '      document.getElementById("downloadDetail").onclick = downloadDetailFile;',
  '      document.getElementById("shareDetail").onclick = shareDetailFile;',
  '      document.getElementById("directLinkDetail").onclick = getDetailDirectLink;',
  '      document.getElementById("renameDetail").onclick = renameDetailItem;',
  '      document.getElementById("deleteDetail").onclick = deleteDetailItem;',
  ...
  '      else if (videoExts.indexOf(ext) !== -1) { container.innerHTML = \'<div id="video-player"></div>\'; new MuiPlayer({ container: "#video-player", title: item.name, src: "/api/download?fileId=" + item.id, autoplay: false, width: "100%", height: "auto" }); }',
  '      else if (audioExts.indexOf(ext) !== -1) { container.innerHTML = \'<audio controls src="/api/download?fileId=\' + item.id + \'" style="width:100%;"></audio>\'; }',
  ...
  '          if (data.base64) { var content = atob(data.base64); container.innerHTML = \'<div class="text-preview">\' + escapeHtml(content) + \'</div>\'; }',
  '        }).catch(function() { container.innerHTML = "<p>无法加载文本预览</p>"; });',
  '      } else { container.innerHTML = "<p>该文件类型不支持预览，请下载后查看。</p>"; }',
  '    }',
  '',
  '    function downloadDetailFile() { if (currentDetailItem) downloadFile(currentDetailItem.id); }',
  '    function shareDetailFile() { if (currentDetailItem) shareFile(currentDetailItem.id); }',
  '    function getDetailDirectLink() {',
  '      if (currentDetailItem) { var link = location.origin + "/d/" + currentDetailItem.id; showModal(\'<h2>直链</h2><div class="share-link-box">\' + link + \'</div><button class="btn btn-primary" onclick="closeModal()">关闭</button>\'); }',
  '    }',
  '    function renameDetailItem() { if (currentDetailItem) showRenameModal(currentDetailItem.id); }',
  '    function deleteDetailItem() { if (currentDetailItem) deleteItem(currentDetailItem.id); }',
  ...
  '      var toolbarConfig = {};',
  '      createToolbar({ editor: editor, selector: "#toolbar-container", config: toolbarConfig, mode: "default" });',
  '      api("/api/file-content?path=" + encodeURIComponent(joinPath(currentPath, item.name))).then(function(data) { if (data.base64) { editor.setHtml(atob(data.base64)); } });',
  ...
  '      var item = currentDetailItem;',
  '      if (item) {',
  '        api("/api/save-content", { method:"POST", body: { path: joinPath(currentPath, item.name), base64: base64, fileId: item.id } }).then(function() { closeModal(); loadTree(); showToast("保存成功"); renderDetail(); }).catch(function(e) { showToast("保存失败: " + e.message); });',
  '      }',
  '    }',
  '',
  '    // ========== 模态框 ==========',
...
// 后端代码（与之前版本相同，此处省略以保证简洁，直接复制之前可用的后端代码即可）
// ... 后端逻辑保持不变 ...
// ==================== 后端逻辑 ====================
