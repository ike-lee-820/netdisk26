...
const HTML = [
  ...
  '        var taskId = taskData.taskId; showToast("开始上传 " + files.length + " 个文件");',
  '        var queue = Promise.resolve(); var completed = 0;',
  '        for (var i = 0; i < files.length; i++) {',
  '          (function(file) { queue = queue.then(function() { return uploadSingleFile(file, file.name, uploadPath, taskId, function(progress) { var overall = ((completed + progress) / files.length) * 100; api("/api/task/update", { method:"POST", body: { taskId: taskId, progress: Math.round(overall) } }); }).then(function() { completed++; api("/api/task/update", { method:"POST", body: { taskId: taskId, progress: Math.round((completed / files.length) * 100) } }); }); }); })(files[i]);',
  '        }',
  ...
  '      var actionsHtml = \'<button class="btn btn-primary" id="downloadDetail">下载</button>\' + \'<button class="btn btn-secondary" id="shareDetail">分享</button>\' + \'<button class="btn btn-secondary" id="directLinkDetail">获取直链</button>\' + \'<button class="btn btn-secondary" id="renameDetail">重命名</button>\' + \'<button class="btn btn-danger" id="deleteDetail">删除</button>\';',
  '      var ext = (item.name.split(".").pop() || "").toLowerCase(); if (["txt","md","json","js","ts","css","html","xml","log"].indexOf(ext) !== -1) actionsHtml += \'<button class="btn btn-secondary" id="editDetail">编辑</button>\';',
  '      card.innerHTML = \'<div class="detail-header"><div class="file-icon \' + getIconClass(item) + \'" style="width:48px;height:48px;font-size:24px;">\' + getFileIcon(item) + \'</div><div class="detail-title">\' + item.name + \'</div></div>\' +',
  ...
  '      document.getElementById("directLinkDetail").onclick = function() { var link = location.origin + "/d/" + item.id; showModal(\'<h2>直链</h2><div class="share-link-box">\' + link + \'</div><button class="btn btn-primary" onclick="closeModal()">关闭</button>\'); };',
  ...
  '      var textExts = ["txt","md","json","js","ts","css","html","xml","log"]; var imageExts = ["jpg","jpeg","png","gif","webp","svg","bmp","ico"]; var videoExts = ["mp4","webm","ogg","mov"]; var audioExts = ["mp3","wav","ogg","flac","m4a"];',
